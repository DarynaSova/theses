...
def get_simulator(dataset_id: ALSimulatorDataset,
                  ...
                  disorder_weight: float = 1.0) -> ActiveLearningSimulator:
    simulation_data = read_FASTA(dataset_id.to_path())
    assert len(simulation_data) > 0, f"Simulation data for {dataset_id} is empty."

    # Load per-residue disorder scores from FASTA file if it exists.
    # Expected file location: datasets/<dataset_name>.fasta.disorder
    # Expected format: >seq_id TARGET=label
    #                  value1, value2, value3, ...
    dataset_fasta_path = dataset_id.to_path()
    disorder_file_path = Path(str(dataset_fasta_path) + ".disorder")
    disorder_raw_dict = {}
    if disorder_file_path.exists():
        try:
            # Load raw per-residue disorder (do NOT aggregate yet)
            # This allows acquisition_step to choose the aggregation method
            from disorder_loader import parse_disorder_fasta
            disorder_raw_dict = parse_disorder_fasta(disorder_file_path)
            print(f"✓ Loaded per-residue disorder for {len(disorder_raw_dict)} sequences from {disorder_file_path}")
        except Exception as e:
            print(f"⚠ Warning: Could not load disorder file {disorder_file_path}: {e}")

    def _lookup_disorder(seq_id: str):
        if not disorder_raw_dict:
            return None
        if seq_id in disorder_raw_dict:
            return disorder_raw_dict[seq_id]

        normalized_dot = seq_id.replace("_", ".")
        if normalized_dot in disorder_raw_dict:
            return disorder_raw_dict[normalized_dot]

        normalized_underscore = seq_id.replace(".", "_")
        if normalized_underscore in disorder_raw_dict:
...
class ActiveLearningSimulator:
    ...
    def _run_simulation(self, model_type: ActiveLearningModelType, embedder_name: str,
        ...
        al_campaign_config = ActiveLearningScreeningCampaignConfig(name="Test",  # TODO
                                                                   ...
                                                                   use_disorder_in_acquisition=self.use_disorder,
