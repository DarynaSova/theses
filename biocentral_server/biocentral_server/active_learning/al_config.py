...
class ActiveLearningScreeningCampaignConfig(BaseModel):
    ...
    """Configuration for disorder in acquisition function"""
    use_disorder_in_acquisition: bool = Field(
        default=False,
        description="If true, include per-sequence disorder aggregates in the acquisition score.",
    )
    disorder_aggregation_method: str = Field(
        default="mean",
        description="Aggregation method for per-residue disorder scores (mean, median, max, p90, frac_above_5, etc.)",
    )
    disorder_weight: float = Field(
        default=1.0,
        description="Weight (gamma) applied to disorder in acquisition = desirability + beta*uncertainty + gamma*disorder.",
        ge=0.0,
    )
...
class ActiveLearningScreeningIterationConfig(BaseModel):
    ...
    ### Change: Add new fields for disorder
    use_disorder_in_acquisition: bool = Field(
    default=False,
    description="If true, include per-sequence disorder in the acquisition score.",
)
    disorder_weight: float = Field(
    default=0.0,
    description="Weight (gamma) applied to disorder when computing acquisition = desirability + beta*uncertainty + gamma*disorder.",
)
    disorder_aggregation_method: str = Field(
    default="mean",
    description="Aggregation method for per-residue disorder scores (mean, median, max, p90, frac_above_5, etc.)",
